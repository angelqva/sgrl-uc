...
import { deleteLocal } from "../_actions/action";

export default function ModalDeleteArea({ local }: { local: Local }) {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const [pending, startTransaction] = useTransition();
  const [state, formAction] = useActionState(deleteLocal, {});
  const handleConfirmar = () => {
    const formData = new FormData();

    formData.append("codigo", local.codigo);
