/*
  Warnings:

  - You are about to drop the column `nombre_completo` on the `Usuario` table. All the data in the column will be lost.
  - Added the required column `nombreCompleto` to the `Usuario` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Usuario" DROP COLUMN "nombre_completo",
ADD COLUMN     "nombreCompleto" TEXT NOT NULL;
